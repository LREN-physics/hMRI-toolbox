...
%                 end
    ...
    for ccon = 1:mpm_params.ncon
        ...
        reg = zeros(nechoes(ccon)-2,2);
        % only if enough echoes
        if mpm_params.estaticsR2s(ccon)
            reg(:,1) = 1;
            reg(:,2) = mpm_params.input(ccon).TE(3:end);%%NC ADD
        end
            
            W = (reg'*reg)\reg';
            
            spm_progress_bar('Init',dm(3),'****** NC Change : Extrapolation to TE=0 for a given contrast ******','planes completed');
            for p = 1:dm(3),
                M = spm_matrix([0 0 p 0 0 0 1 1 1]);
                data = zeros([nechoes(ccon)-2 dm(1:2)]);%%NC ADD
                ...
                if mpm_params.estaticsR2s(ccon)
                    ...
                    for cecho = 3:nechoes(ccon)%%NC ADD
                        % Third order B-spline interpolation for OLS R2* estimation
                        % since we no longer assume that the echoes are perfectly
                        % aligned as we do for the standard PDw derived R2*
                        % estimate.
                        data(cecho-2,:,:) = log(max(spm_slice_vol(Vcon(cecho),M1,dm(1:2),mpm_params.interp),1));
                    end
                end
                
                Y = W*reshape(data, [nechoes(ccon)-2 prod(dm(1:2))]);%% NC ADD
                ...
                if mpm_params.fullOLS
                        ...
                        % only if enough echoes
                        if mpm_params.estaticsR2s(ccon)
                            Nmap(ccon).dat(:,:,p) = reshape(exp(Y(1,:)), dm(1:2));
    ...
    if (mpm_params.T1widx && mpm_params.PDwidx)
        ...
        if mpm_params.fullOLS
            mpm_params.output(coutput).descrip{end+1} = '- R2* bias accounted for by TE=0 extrapolation (fullOLS option) **per contrast NC**';
